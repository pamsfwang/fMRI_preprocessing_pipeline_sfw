function art_slice_sfw(imgDir,imgFilter,repairOption,threshold,mask_name)
%====================================================================================
%====================================================================================
...
%FORMAT art_slice_sfw  (v.3)(Shao-Fang Wang 2016)
%art_slice_sfw(imgDir,imgFilter,repairOption,threshold)
%imgDir: Path where functional images live
%imgFilter: wild card: '^ra.*nii'
%repairOption:
%threshold: (default = 18) Threshold above sample means to filter slices
%pq = 0.5*( min(p) + median(p,1));
%pq = pq + threshold; --> the final threshold being used

%line112: change it to user mask.
%Y = art_automask( Image, Threshold, WriteVol )      (v2.3)
%when applying threshold, the mask only contain the brain and part of the
%scalp (auto mask results)
%Automask left a lot of scalp outside of the mask
...
% PARAMETERS
spm_defaults;
OUTSLICEdef =  threshold;  %  Threshold above sample means to filter slices
...
list = spm_select('FPList',imgDir,imgFilter);%list = spm_select('FPList',pwd,'^ra.*');
P = spm_vol(list);
...
fprintf('\n AutoMask %d, allfilt %d, slicefile %d\n',amask,allfilt,slicefilt)
...
if amask == 1 | slicefilt == 1 %  Automask options
    ...
    Automask = spm_read_vols(spm_vol(mask_name));%user mask
