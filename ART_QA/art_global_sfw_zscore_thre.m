function [z_thresh,pers_thresh, out_idx,num_fimage]= art_global_sfw_zscore_thre(GUI,zscore_thresh,per_thresh,GoRepair)%(Images,RealignmentFile,HeadMaskType, RepairType)
%====================================================================================
%====================================================================================
% Run through each run for each subject to calculate outliers based on z
% score or percentage of mean signal-sfw 2017 (adapt the art_global script)
% If input zscore_thresh is 0, then the function uses percent mean signal
% as threshold. If input zscore_thresh is above 0, then the function use
% the number as # of SDs as threshold.
%
% zscore_thresh: use SD as theshold. Enter the number of SDs you want to use as threshold.
% Exp: zscore_thresh=3; 3 SDs as threshold
% per_thresh: use percent mean signal as threshold. Enter the number of percentage you want to use as threshold.
% Exp: per_thresh: 1.3; 1.3% of mean as threshold
% line 290, 291: these two lines transform cut-off BOLD signal
% threshold values into std values. We comment out these two lines to make
% it uses the std value we define as the cut-off threshold
%%z_thresh = mincount/gsigma;        % Default value is PercentThresh.
%%z_thresh = 0.1*round(z_thresh*10); % Round to nearest 0.1 Z-score value
%
% Output: 
% 1. z_thresh: SD threshold
% 2. pers_thresh: corresponding percentage of mean signal for the SD threshold
% 3. out_idx: outlier volums (time points)
% 4. number of functional images
...
%Check spm version: [ dummy, spm_ver ] = fileparts(spm('Dir'));
%
...
Percent_thresh = per_thresh;
%  Alternatively, deviations over 2*std are outliers, if std is not very small.
z_thresh = zscore_thresh;  % Currently not used for default.
...
%% For auto collecting files - specify parameters
num_sess = 1; %session number
global_type_flag = 3;%3 usermasks; 4 generate a mask
repair1_flag = 0; %repair 1st scan? y==1, n==0
mask_fnmae = 'temp.*nii'; %whole brain mask fname
func_fname = '^rag.*nii'; %func images fname
mv_fname = '^rp.*txt'; %motion parameter fname
...
%%%%%%Collect file Auto%%%%%
    ...
    if global_type_flag==3
        fprintf('User Mask  ( in same voxel space as functional images )\n')
        if strcmp(spm_ver,'spm2')==0
            mask = spm_select('FPList',pwd,mask_fnmae);
    ...
    if strcmp(spm_ver,'spm2')==0
        for i = 1:num_sess
            P{i} = spm_select('FPList',pwd,func_fname);
            num_fimage = size(P{i},1);
            fprintf('Functional images %03d\n',num_fimage)
            
            if realignfile == 1
                mvmt_file = spm_select('FPList',pwd,mv_fname);
                M{i} = load(mvmt_file);
            end
...
if z_thresh ==0
fprintf('\n Use signal percentage as threshold\n')
z_thresh = mincount/gsigma;        % Default value is PercentThresh.
z_thresh = 0.1*round(z_thresh*10); % Round to nearest 0.1 Z-score value
end
...
%Print & Save out index
pers_thresh = z_thresh*pctmap;
fprintf('\n zthre %f percent thre %f \n', z_thresh,pers_thresh)
fprintf('\n outliers: %d\n',length(out_idx));out_idx
