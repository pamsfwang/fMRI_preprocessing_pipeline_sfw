%Shao-Fang Wang 2016
%Modifoed from art_global

function []=art_global_raw_sfw(b) 
num_sess = length(b.funcRuns);
global_type_flag=4; 
%which global mean to use:
%1. Regular SPM mask
%4. Auto (generates ArtifactMask and can Calculate Movement)
%3. User Mask
%2. Every voxel

realignfile = 0;
%Have realignment files? 1: yes 0:no

imgFilter = '.*nii';
%Select functional images

mvmtFilter = '.*txt';
%Select motion parameter file

repair1_flag = 0;
%Always repair 1st scan of each session? 1: yes 0:no

figure_name = 'art_global_raw_';
out_idx_name = 'outlier_raw_idx_';
...
z_thresh = 3;  % Currently not used for default.
...
%         [1 4 3 2], 2);
    if global_type_flag==3
        error('Set up mask dir in the script! \n')
...
%     end
    ...
    if strcmp(spm_ver,'spm2')==0
        for i = 1:num_sess
            P{i} = spm_select('FPList',[b.dataDir,b.funcRuns{i}],imgFilter);
            if realignfile == 1
                mvmt_file = spm_select('FPList',imgDir,mvmyFilter);
                M{i} = load(mvmt_file);
            end
        end
    else   % spm2 version
...
figure('Units', 'normalized','Position',[0.3 0.3 0.7 0.8]);
...
if realignfile == 1
    ...
    legend('x mvmt', 'y mvmt', 'z mvmt','pitch','roll','yaw',0,'Location','East');
    h = gca;
    set(h,'Ygrid','on');
elseif realignfile == 0
    ...
    legend('x mvmt', 'y mvmt', 'z mvmt',0,'Location','East');
    h = gca;
    set(h,'Ygrid','on');
end
...
y_lim = get(gca, 'YLim');
legend('Fast motion',0,'Location','East');
h = gca;
set(h,'Ygrid','on');
...
descr = {['StdDev of data is: ',num2str(gsigma)];
    ['Current threshold (std devs):',num2str(z_thresh)];
    ['Motion threshold  (mm / TR):',num2str(mv_thresh)];
    ['Current threshold (% of mean):',num2str(z_thresh*pctmap)];
    ['Outlier indices: ',int2str(length(out_idx))]}

out_idx

text(.025,-7,descr)
% Save figure
saveas(figure(1),[b.dataDir,[figure_name,b.curSubj]],'jpg')
save([b.dataDir,[out_idx_name,b.curSubj]],'out_idx')
